...
class Delivery(models.Model):
    _name = 'autopos.delivery'
    _rec_name = 'name'
    _order = 'name desc'
    name = fields.Char('Name')
    date_delivery = fields.Date('Date Delivery', required=True, default=fields.Date.today())
    sale_id = fields.Many2one('autopos.sale', string='Sale', required=True)
    customer_id = fields.Many2one('autopos.customer', related='sale_id.customer_id', string='Customer')
    ...
    delivery_line = fields.One2many('autopos.delivery_line', 'delivery_id')

    @api.model
    def create(self, vals):
        seq = self.env['ir.sequence'].next_by_code('autopos.delivery_no') or '-'
        vals['name'] = seq
    ...
    def copy(self, default=None):
        ...
        return super(Delivery, self).copy(default=default)

    @api.onchange('delivery_line')
    def get_total_amount(self):
        subtotal = 0
        for r in self:
            for item in r.delivery_line:
    ...
    def do_delivery_approve(self):
        return {
            'view_type': 'form',
            'view_mode': 'form',
            'res_model': 'autopos.delivery.approve.wizard',
            'target': 'new',
    ...
    def do_delivery_cancel(self):
        return {
            'view_type': 'form',
            'view_mode': 'form',
            'res_model': 'autopos.delivery.cancel.wizard',
            'target': 'new',
            'type': 'ir.actions.act_window',
            'context': {
                'default_rcv_id': self.id,
    ...
    def do_delivery_print(self):
        return {
            'type': 'ir.actions.report',
            'report_name': 'autopos_inventory.delivery_form',
            'model': 'autopos.delivery',
            'report_type': 'qweb-pdf',
        }

    def do_delivery_close(self):
        self.write({'state': 'close'})


class DeliveryLine(models.Model):
    _name = 'autopos.delivery_line'
    delivery_id = fields.Many2one('autopos.delivery', ondelete='cascade')
