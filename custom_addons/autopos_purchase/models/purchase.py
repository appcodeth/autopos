...
class Purchase(models.Model):
    ...
    supplier_id = fields.Many2one('autopos.supplier', string='Supplier', required=True)
    ...
    purchase_line = fields.One2many('autopos.purchase_line', 'purchase_id')

    @api.model
    def create(self, vals):
        seq = self.env['ir.sequence'].next_by_code('autopos.purchase_no') or '-'
        vals['name'] = seq
    ...
    def do_purchase_print(self):
        ...
        return {
            ...
            'report_name': 'autopos_purchase.purchase_form',
            'model': 'autopos.purchase',
            'report_type': 'qweb-pdf',
        }

    def do_purchase_receive(self):
        return {
            'view_type': 'form',
            'view_mode': 'form',
            'res_model': 'autopos.po.receive.wizard',
            'target': 'new',
    ...
    def do_purchase_email(self):
        ...
        try:
            template_id = ir_model_data.get_object_reference('autopos_purchase', 'mail_template_purchase_form')[1]
        except ValueError:
            template_id = False
        try:
        ...
        ctx = {
            ...
            'default_model': 'autopos.purchase',
...
class PurchaseLine(models.Model):
    _name = 'autopos.purchase_line'
    purchase_id = fields.Many2one('autopos.purchase', ondelete='cascade')
