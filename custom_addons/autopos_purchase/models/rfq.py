...
class RFQ(models.Model):
    ...
    rfq_line = fields.One2many('autopos.rfq_line', 'rfq_id')

    @api.model
    def create(self, vals):
        seq = self.env['ir.sequence'].next_by_code('autopos.rfq_no') or '-'
        vals['name'] = seq
    ...
    def do_rfq_print(self):
        ...
        return {
            ...
            'report_name': 'autopos_purchase.rfq_form',
            'model': 'autopos.rfq',
    ...
    def do_rfq_approve(self):
        return {
            'view_type': 'form',
            'view_mode': 'form',
            'res_model': 'autopos.rfq.approve.wizard',
            'target': 'new',
    ...
    def do_rfq_cancel(self):
        return {
            'view_type': 'form',
            'view_mode': 'form',
            'res_model': 'autopos.rfq.cancel.wizard',
            'target': 'new',
            'type': 'ir.actions.act_window',
            'context': {
                'default_rfq_id': self.id,
    ...
    def do_rfq_email(self):
        ...
        try:
            template_id = ir_model_data.get_object_reference('autopos_purchase', 'mail_template_rfq_form')[1]
        except ValueError:
            template_id = False
        try:
...
class RFQLine(models.Model):
    _name = 'autopos.rfq_line'
    rfq_id = fields.Many2one('autopos.rfq', ondelete='cascade')
