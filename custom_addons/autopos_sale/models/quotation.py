...
class Quotation(models.Model):
    ...
    quotation_line = fields.One2many('autopos.quotation_line', 'quotation_id')

    @api.model
    def create(self, vals):
        seq = self.env['ir.sequence'].next_by_code('autopos.quotation_no') or '-'
        vals['name'] = seq
    ...
    def do_quotation_print(self):
        self.ensure_one()
        return {
            'type': 'ir.actions.report',
            'report_name': 'autopos_sale.quotation_form',
            'model': 'autopos.quotation',
            'report_type': 'qweb-pdf',
        }

    def do_quotation_approve(self):
        return {
            'view_type': 'form',
            'view_mode': 'form',
            'res_model': 'autopos.quotation.approve.wizard',
            'target': 'new',
            'type': 'ir.actions.act_window',
            'context': {
                'default_quotation_id': self.id,
                'default_quotation_name': self.name,
            }
        }

    def do_quotation_cancel(self):
        return {
            'view_type': 'form',
            'view_mode': 'form',
            'res_model': 'autopos.quotation.cancel.wizard',
            'target': 'new',
            'type': 'ir.actions.act_window',
            'context': {
                'default_quotation_id': self.id,
    ...
    def do_quotation_close(self):
        self.write({'state': 'close'})

    def do_quotation_email(self):
        self.ensure_one()
        ir_model_data = self.env['ir.model.data']
        try:
            template_id = ir_model_data.get_object_reference('autopos_sale', 'mail_template_quotation_form')[1]
        except ValueError:
            template_id = False
        try:
        ...
        ctx = {
            ...
            'default_model': 'autopos.quotation',
...
class QuotationLine(models.Model):
    _name = 'autopos.quotation_line'
    quotation_id = fields.Many2one('autopos.quotation', ondelete='cascade')
